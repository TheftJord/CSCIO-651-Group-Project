...
public class PrimaryController {
    ...
    public void initialize() {
        ...
        DescCol.setCellValueFactory(new PropertyValueFactory<PartView, String>("description"));
        ; // sets up Part Description Column
    }
    ...
    private void tabPaneRemove() throws IOException {
        ...
        String[] splitText = partID.split(",");

        for (String part : splitText) {
            try {
                // deletes part from data structure
                BPlusTree.delete(part.trim()); // removes part from data structure by part ID
            } catch (Exception e) {
            }
        }
    ...
    private void tableViewAddItems() {
        ...
        for (Part temp : BPlusTree.displayNext10(holder)) { // will take items from data structure
            PartView Swapper = new PartView(temp.getPartId(), temp.getDescription()); // intermediatary because parts
                                                                                      // had an implimentation
